...
describe('Generate tx2 types', () => {
    it('MF-e', () => {
        if (!fs.existsSync('__test__/files/mfe/tx2'))
            fs.mkdirSync('__test__/files/mfe/tx2', { recursive: true });
        const tx2FileName = 'test.tx2';
        const tx2 = index_1.generateMFeTx2(`__test__/files/mfe/tx2/${tx2FileName}`, invoiceInformation, [invoiceItems], [invoicePayment], invoiceTechnical);
        expect(tx2).toBe(tx2);
    });
});
